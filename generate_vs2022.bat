...
if not defined BUILD_DIR set "BUILD_DIR=build-vs2022"
...
if not exist "%SCRIPT_DIR%\%BUILD_DIR%" (
    mkdir "%SCRIPT_DIR%\%BUILD_DIR%"
    if errorlevel 1 (
        echo [ERROR] Failed to create build directory: "%SCRIPT_DIR%\%BUILD_DIR%"
        exit /b 1
    )
)

echo [INFO] Source dir : "%SOURCE_DIR%"
echo [INFO] Build dir  : "%SCRIPT_DIR%\%BUILD_DIR%"
...
cmake -S "%SOURCE_DIR%" -B "%SCRIPT_DIR%\%BUILD_DIR%" -G "Visual Studio 17 2022" -A x64 "-DCMAKE_PREFIX_PATH=%QT_DIR%"
if errorlevel 1 (
...
echo [INFO] Open "%SCRIPT_DIR%\%BUILD_DIR%\SARibbon.sln"
echo [INFO] Or build with:
echo        cmake --build "%SCRIPT_DIR%\%BUILD_DIR%" --config %CONFIG_NAME%
